...
type PDFAction =
  ...
  | {
      type: "UPDATE_FILE";
      payload: { id: string; updates: Partial<PDFFileInfo> };
    }
...
export function PDFProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      dispatch({
        type: "SET_ERROR",
        payload: "Failed to initialize PDF service. Please refresh.",
      });
    }
  ...
  // Generate thumbnail using pdfjs-dist (lazy loaded) or custom logic
      ...
      try {
        ...
        if (type === "text") {
          ...
          if (ctx) {
            ...
            ctx.fillRect(0, 0, 96, 136);
            ctx.strokeStyle = "#dee2e6";
            ctx.strokeRect(0, 0, 96, 136);
            ctx.fillStyle = "#495057";
            ctx.font = "bold 16px Arial";
            ctx.textAlign = "center";
            ctx.textBaseline = "middle";
            ctx.fillText("TXT", 48, 68);
  ...
  const getPageCount = useCallback(
    async (buffer: ArrayBuffer, type?: string): Promise<number> => {
      return new Promise((resolve) => {
        ...
        workerRef.current.postMessage(
          {
            type: "getPageCount",
            payload: { buffer: bufferToTransfer, type },
            id,
          },
          [bufferToTransfer],
        );
  ...
  const addFiles = useCallback(
    async (files: File[]) => {
      const validFiles = files.filter(isMergeableFile);
      ...
      const newFiles: PDFFileInfo[] = validFiles.map((file) => ({
        ...
        error: null,
        arrayBuffer: null,
        type: getFileCategory(file) as "pdf" | "image" | "text",
      }));
      ...
      for (const fileInfo of newFiles) {
        try {
          ...
          const [pageCount, thumbnailUrl] = await Promise.all([
            getPageCount(buffer, fileInfo.type),
            generateThumbnail(buffer, fileInfo.file, fileInfo.type),
          ]);
  ...
  const moveFileToTop = useCallback(
    (id: string) => {
      ...
      if (index > 0) {
        dispatch({
          type: "MOVE_FILE",
          payload: { fromIndex: index, toIndex: 0 },
        });
      }
    },
    [state.files],
  ...
  const mergePDFs = useCallback(async () => {
    ...
    if (!workerRef.current) {
      dispatch({
        type: "SET_ERROR",
        payload: "Worker not available. Please refresh.",
      });
      return;
    }
    ...
    const filesToMerge = await Promise.all(
      state.files.map(async (file) => {
        ...
        console.log(
          `Preparing "${file.name}" for merge. Size: ${buffer.byteLength} bytes`,
        );
        return {
          id: file.id,
    ...
    dispatch({
      ...
      payload: {
        status: "merging",
        progress: 30,
        message: "Merging PDFs... This may take a moment.",
      },
    });
