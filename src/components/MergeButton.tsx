...
export function MergeButton() {
  ...
  const isMerging =
    mergeProgress.status === "merging" || mergeProgress.status === "loading";
  ...
  const getButtonText = () => {
    ...
    if (files.length === 0) return "Add Files to Merge";
    if (files.length === 1) return "Add Another File";
        ...
        >
          <Flex direction="column" align="center" gap={3}>
            <Button
              size="xl"
              onClick={
                isError ? mergePDFs : isMerging ? cancelMerge : mergePDFs
              }
              disabled={isDisabled && !isError}
              bg={
                isDisabled
                  ? "gray.300"
                  : `linear-gradient(135deg, ${getButtonColor()}.500 0%, ${getButtonColor()}.600 100%)`
              }
              color="white"
              px={12}
              py={6}
