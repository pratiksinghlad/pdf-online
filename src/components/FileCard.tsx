...
export function FileCard({ file, index }: FileCardProps) {
    const { removeFile, moveFileToTop, moveFileToBottom, moveFile, files } = usePDF();
    ...
    return (
        <MotionBox
            ...
            tabIndex={0}
            role="listitem"
            aria-label={`PDF file: ${file.name}, ${file.pageCount} pages, ${formatFileSize(file.size)}. Use arrow keys to reorder, Delete to remove.`}
            _focus={{
                outline: '2px solid',
