...
import { PdfToImageDropZone, PdfToImageOptions, PagePreviewGrid, ExportActions } from '../components';
import { PdfToImageProvider, usePdfToImage } from '../context/PdfToImageContext';
...
const FEATURES = [
    {
        ...
        description:
            'Render PDF pages at up to 3× resolution for crystal-clear images. Choose between PNG (lossless) or JPG (compact).',
    },
    {
        emoji: '🔒',
        bg: 'green.100',
        title: '100% Private',
        description:
            'All conversion happens in your browser. Your PDF is never uploaded to any server — complete privacy guaranteed.',
    },
    {
        emoji: '⚡',
        bg: 'blue.100',
        ...
        description:
            'Convert all pages at once, select individual pages to download, or export everything as a ZIP archive.',
    },
] as const;
...
function LoadedFileCard() {
    const { file, removeFile, pages } = usePdfToImage();
                ...
                <HStack gap={3} minW={0}>
                    ...
                    <Box minW={0}>
                        ...
                        <HStack gap={2} mt={0.5}>
                            ...
                            {pages.some((p) => p.imageBlob) && (
                                <Badge colorPalette="green" fontSize="xs" borderRadius="md">
                                    {pages.filter((p) => p.imageBlob).length} rendered
                                </Badge>
                            )}
                            <Text fontSize="xs" color="gray.500">
...
function PdfToImagePageContent() {
    const { file } = usePdfToImage();
    ...
    return (
        <Box minH="100vh" bg="white">
            ...
            {/* Workspace — visible after PDF is loaded */}
            {file && (
                <Container maxW="1200px" px={{ base: 4, md: 6 }} py={{ base: 6, md: 10 }}>
                    <VStack gap={{ base: 4, md: 6 }} align="stretch">
                        ...
                        <Box
                            display="grid"
                            gridTemplateColumns={{ base: '1fr', md: '1fr 320px' }}
                            gap={6}
                            alignItems="start"
                        >
